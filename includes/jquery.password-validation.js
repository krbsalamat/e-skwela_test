	...
	$.fn.extend({
		passwordValidation: function(_options, _callback, _confirmcallback) {
			...
			var _defaults = {
				...
				maxRepeats: 8,		  //Maximum number of repeated alphanumeric characters in password dhgurAAAfjewd <- 3 A's
				maxConsecutive: 8,	  //Maximum number of alphanumeric characters from one set back to back
			...
			function buildPasswordRegex() {
				...
				else 					cases.push({"regex": "(?=" + ("[" + CHARSETS.lowerCaseSet + "][^" + CHARSETS.lowerCaseSet + "]*").repeat(_options.minLowerCase) + ")", 	"message": "" + _options.minLowerCase + " Lower Case Letters."});
				if(_options.noDigit) 	cases.push({"regex": "(?=" + CHARSETS.digitSet + ")", 																					"message": "Password can't contain a Number"});
				else 					cases.push({"regex": "(?=" + ("[" + CHARSETS.digitSet + "][^" + CHARSETS.digitSet + "]*").repeat(_options.minDigits) + ")", 			"message": "" + _options.minDigits + " Digit"});
